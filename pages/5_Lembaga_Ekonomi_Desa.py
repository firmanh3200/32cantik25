...
with st.container(border=True):
    with st.container(border=True):
        st.title(":green[Lembaga Ekonomi di Desa]")
...
# LEMBAGA KEUANGAN
with st.expander('LEMBAGA KEUANGAN'):    
    if kabterpilih and kecterpilih and tahunterpilih:
        url = f'https://portaldatadesa.jabarprov.go.id/api/idm/idmdataidentitas/v2?sub_name=Akses%20Lembaga%20Keuangan&tahun={tahunterpilih}&page=1&limit=100&order=asc&orderby=id_desa&id_kabupaten={kabterpilih}&id_kecamatan={kecterpilih}'
...
# PUSAT PERDAGANGAN
with st.expander('PUSAT PERDAGANGAN'):    
    if kabterpilih and kecterpilih and tahunterpilih:
        url = f'https://portaldatadesa.jabarprov.go.id/api/idm/idmdataidentitas/v2?sub_name=Akses%20Ke%20Pusat%20Perdagangan&tahun={tahunterpilih}&page=1&limit=100&order=asc&orderby=id_desa&id_kabupaten={kabterpilih}&id_kecamatan={kecterpilih}'
        response = requests.get(url)
        data = response.json()
...
# LEMBAGA EKONOMI
with st.expander('LEMBAGA EKONOMI'):    
    if kabterpilih and kecterpilih and tahunterpilih:
        url = f'https://portaldatadesa.jabarprov.go.id/api/idm/idmdataidentitas/v2?sub_name=Ketersediaan%20Lembaga%20Ekonomi&tahun={tahunterpilih}&page=1&limit=100&order=asc&orderby=id_desa&id_kabupaten={kabterpilih}&id_kecamatan={kecterpilih}'
        response = requests.get(url)
        data = response.json()
